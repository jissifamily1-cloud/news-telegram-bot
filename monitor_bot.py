...
def press_name(url, source="", cache=None):
    ...
    for d, n in PRESS_MAP.items():
        if h == d or h.endswith("." + d):
            return n
    if cache and h in cache and cache[h] != h:
        return cache[h]
    return None  # 미해결 → 동적 조회 대상
    ...
    try:
        if first:
            ...
            for (title, url, source, desc, k, tk, toks) in accepted[:MAX_SEND_PER_RUN]:
                nm = press_name(url, source, pcache)
                if nm is None:
                    global _PRESS_BUDGET
                    hh = _host(url) or "기타"
                    if _PRESS_BUDGET > 0:
                        _PRESS_BUDGET -= 1
                        nm = _fetch_site_name(url) or hh
                        pcache[hh] = nm
                    else:
                        nm = hh
...
def main():
    ...
    # 2) 활성 방마다 발송
    pcache = state.setdefault("press_names", {})
